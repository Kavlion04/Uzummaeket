...
async function loadDetail() {
  ...
  product.images.forEach((img, idx) => {
    gallery += `<img src="${img}" class="${idx === 0 ? 'active' : ''}" data-img="${img}">`;
  });
  ...
  document.getElementById('detail').innerHTML = `
    ...
    <div class="detail-info">
        ...
        <div class="detail-rating">
            ${getStars(product.rating)}
            <span style="color:#62656a;font-size:16px;margin-left:6px">${product.rating}</span>
        </div>
  ...
  document.querySelectorAll('.detail-gallery img').forEach(img => {
    img.addEventListener('click', function() {
      ...
      document.querySelectorAll('.detail-gallery img').forEach(i => i.classList.remove('active'));
...
async function loadDetail() {
  ...
  let gallery = '';
  product.images.forEach((img, idx) => {
    gallery += `<img src="${img}" class="${idx === 0 ? 'active' : ''}" data-idx="${idx}">`;
  });
